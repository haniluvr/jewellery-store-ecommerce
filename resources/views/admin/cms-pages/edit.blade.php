...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-stone-900 dark:text-white">Edit CMS Page</h1>
                    <p class="mt-1 text-sm text-stone-600 dark:text-gray-400">Edit "{{ $cmsPage->title }}"</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ admin_route('cms-pages.preview', $cmsPage) }}" target="_blank" 
    ...
    <form action="{{ admin_route('cms-pages.update', $cmsPage) }}" method="POST" enctype="multipart/form-data" class="space-y-8" id="cmsForm">
        @csrf
        @method('PUT')
                ...
                <div id="content-panel" class="tab-panel">
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            >{{ old('excerpt', $cmsPage->excerpt) }}</textarea>
                            @error('excerpt')
                                <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea
                                ...
                                required
                            >{{ old('content', $cmsPage->content) }}</textarea>
                            @error('content')
                                <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="mt-1 flex justify-between text-xs text-stone-500 dark:text-gray-400">
                                ...
                                <span id="meta_title_count">{{ strlen(old('meta_title', $cmsPage->meta_title ?? '')) }}/60</span>
                            </div>
                            @error('meta_title')
                                <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea
                                ...
                                maxlength="160"
                            >{{ old('meta_description', $cmsPage->meta_description) }}</textarea>
                            <div class="mt-1 flex justify-between text-xs text-stone-500 dark:text-gray-400">
                                <span>Recommended: 150-160 characters</span>
                                <span id="meta_description_count">{{ strlen(old('meta_description', $cmsPage->meta_description ?? '')) }}/160</span>
                            </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input
                                ...
                                id="meta_keywords"
                                name="meta_keywords"
                                value="{{ old('meta_keywords', $cmsPage->meta_keywords) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="border border-stone-200 dark:border-strokedark rounded-xl p-4 bg-stone-50 dark:bg-gray-800">
                                <div id="seo-preview-title" class="text-blue-600 dark:text-blue-400 text-lg font-medium hover:underline cursor-pointer">
                                    {{ old('meta_title', $cmsPage->meta_title) ?: $cmsPage->title }}
                                </div>
                                <div id="seo-preview-url" class="text-green-700 dark:text-green-400 text-sm">
                                    {{ url('/') }}/<span id="seo-preview-slug">{{ old('slug', $cmsPage->slug) }}</span>
                                </div>
                                <div id="seo-preview-description" class="text-stone-600 dark:text-gray-400 text-sm mt-1">
                                    {{ old('meta_description', $cmsPage->meta_description) ?: 'Your meta description will appear here' }}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                >
                                    @foreach(\App\Models\CmsPage::getTypeOptions() as $value => $label)
                                        <option value="{{ $value }}" {{ old('type', $cmsPage->type) === $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                            ...
                            <div class="space-y-2">
                                ...
                                >
                                    <option value="0" {{ old('is_active', $cmsPage->is_active) == 0 ? 'selected' : '' }}>Draft</option>
                                    <option value="1" {{ old('is_active', $cmsPage->is_active) == 1 ? 'selected' : '' }}>Published</option>
                                </select>
                                @error('is_active')
                                    <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="space-y-2">
                                ...
                                <input
                                    ...
                                    id="published_at"
                                    name="published_at"
                                    value="{{ old('published_at', $cmsPage->published_at ? $cmsPage->published_at->format('Y-m-d\TH:i') : '') }}"
                            ...
                            <div class="space-y-2">
                                ...
                                <input
                                    ...
                                    id="sort_order"
                                    name="sort_order"
                                    value="{{ old('sort_order', $cmsPage->sort_order) }}"
                                    min="0"
                            ...
                            <div class="lg:col-span-2 space-y-4">
                                ...
                                <div id="featured-image-container" class="relative group">
                                    ...
                                    <div id="image-preview-placeholder" 
                                         ...
                                         class="cursor-pointer border-2 border-dashed border-stone-200 dark:border-strokedark rounded-2xl p-12 transition-all duration-200 hover:border-emerald-500 hover:bg-emerald-50/30 dark:hover:bg-emerald-500/5 group {{ $cmsPage->featured_image ? 'hidden' : '' }}">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-stone-900 dark:text-white font-medium">Click to upload new featured image</p>
                                            <p class="text-sm text-stone-500 dark:text-gray-400 mt-1">or drag and drop here</p>
                                        </div>
                                    </div>

                                    <div id="image-preview-active" class="relative rounded-2xl overflow-hidden border border-stone-200 dark:border-strokedark shadow-lg group {{ $cmsPage->featured_image ? '' : 'hidden' }}">
                                        <img id="featured-preview-img" src="{{ $cmsPage->featured_image_url ?? '#' }}" alt="Preview" class="w-full h-64 object-cover">
                                        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-4">
                                            ...
                                            <button type="button" onclick="removeCurrentFeaturedImage()" class="p-3 bg-red-600 rounded-xl text-white hover:bg-red-700 transition-colors shadow-lg">
                            ...
                            <div class="flex items-center space-x-6">
                                <label class="flex items-center">
                                    <input
                                        ...
                                        name="is_featured"
                                        value="1"
                                        {{ old('is_featured', $cmsPage->is_featured) ? 'checked' : '' }}
